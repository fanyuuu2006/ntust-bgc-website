...
type AvatarManagementProps = { user: Pick<User, "id" | "name" | "avatar">; endpoint: string; admin?: boolean };
...
async function readAvatarResponse(response: Response): Promise<AvatarResponse> {
  const payload = await response.json().catch(() => null) as { data?: { avatar?: unknown }; message?: unknown } | null;
  if (!response.ok) throw new ApiError(typeof payload?.message === "string" ? payload.message : "頭像操作失敗，請稍後再試", response.status);
  if (!payload?.data || !(typeof payload.data.avatar === "string" || payload.data.avatar === null)) throw new ApiError("伺服器回傳的頭像資料格式不正確", 500);
...
export function AvatarManagement({ user, endpoint, admin = false }: AvatarManagementProps) {
  ...
  function resetSelection() {
    setFile(null); setUploadError(null);
    if (inputRef.current) inputRef.current.value = "";
  }
  function closeUpload() {
    if (uploading) return;
    setUploadOpen(false); resetSelection();
  }
  function handleFileChange(event: React.ChangeEvent<HTMLInputElement>) {
    const nextFile = event.target.files?.[0] ?? null;
    if (!nextFile) return;
    const error = nextFile.size === 0 ? "圖片檔案不可為空"
      : nextFile.size > AVATAR_MAX_BYTES ? "圖片檔案不可超過 2 MiB"
        : !ACCEPTED_TYPES.has(nextFile.type) ? "僅支援 JPEG、PNG 或 WebP 圖片" : null;
    setUploadError(error); setFile(error ? null : nextFile);
  ...
  return <section aria-label={admin ? "使用者頭像管理" : "頭像管理"}>
    <div className="flex min-w-0 flex-col gap-3 sm:flex-row sm:items-center">
      ...
      <div className="min-w-0 flex-1">
        ...
        <div className="mt-2 flex flex-wrap items-center gap-2">
          ...
          {avatar ? <Button type="button" variant="text" onClick={() => { setRemoveError(null); setRemoveOpen(true); }}><Trash2 aria-hidden="true" className="size-4" />移除</Button> : null}
        </div>
      </div>
    </div>
    <FormFeedback error={removeOpen ? null : removeError} success={success} className="mt-2" />
    <Modal open={uploadOpen} onClose={closeUpload} closeDisabled={uploading} title={actionLabel} description="選擇一張圖片作為頭像。" size="sm">
      <div className="space-y-4" aria-busy={uploading || undefined}>
        {previewUrl ? <div className="flex min-w-0 items-center gap-3 rounded-xl border border-(--border-default) bg-(--surface-subtle) p-3">
          ...
          <img src={previewUrl} alt="待上傳的頭像預覽" className="size-20 shrink-0 rounded-full object-cover" />
    ...
    <ConfirmDialog open={removeOpen} onClose={() => { if (!removing) setRemoveOpen(false); }} onConfirm={removeAvatar} title={admin ? "移除這位使用者的頭像？" : "移除頭像？"} description="移除後將顯示預設頭像。" confirmLabel="移除頭像" isSubmitting={removing} size="sm"><FormFeedback error={removeError} /></ConfirmDialog>
  </section>;
}
