...
export function MembershipRecordsToolbar({
  ...
  const clearFiltersHref = buildOwnedQueryHref({
    basePath: "/memberships",
    appliedQuery,
    ownedKeys: ["type", "status"],
    changes: {},
  });
  const activeFilterCount = Number(Boolean(query.type)) + Number(Boolean(query.status));
  ...
  return (
    <div className="space-y-2">
      <div className="grid min-w-0 gap-2 lg:grid-cols-[minmax(0,1fr)_auto_auto] lg:items-center">
        ...
        <QueryFilterDisclosure label={activeFilterCount ? `篩選 (${activeFilterCount})` : "篩選"}>
          <QueryFilterForm
            method="GET"
            action="/memberships"
          ...
          >
            <PreservedQueryFields query={appliedQuery} ownedKeys={["type", "status"]} />
