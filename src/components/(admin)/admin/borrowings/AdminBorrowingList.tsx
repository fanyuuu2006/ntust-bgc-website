...
export function AdminBorrowingList({ borrowings, query }: { borrowings: BoardGameBorrowingForAdmin[]; query: BorrowingQuery }) {
  ...
  return <div className="space-y-4">
    ...
    <Modal open={selected?.action === "checkout"} onClose={close} title={actionTitle} description={actionDescription}><div className="space-y-4"><Field label="應還時間" htmlFor="borrowing-due-at"><Input id="borrowing-due-at" autoFocus type="datetime-local" value={dueAt} onChange={(event) => setDueAt(event.target.value)} /></Field><FormFeedback error={feedback} /><div className="flex flex-wrap justify-end gap-2"><Button variant="outline" onClick={close} disabled={busy}>取消</Button><Button onClick={run} disabled={busy} isLoading={busy}>{busy ? "處理中…" : "確認借出"}</Button></div></div></Modal>
