...
export const announcementsService = {
  ...
  updateForAdmin: async (id: AnnouncementId, input: unknown) => { const current = await announcementsRepository.findById(id); if (!current) throw new AnnouncementNotFoundError(); const data = inputSchema.parse(input); return announcementsRepository.updateById(id, { ...data, published_at: data.is_published && !current.is_published ? new Date().toISOString() : data.is_published ? current.published_at : null }); },
  deleteForAdmin: async (id: AnnouncementId) => { const current = await announcementsRepository.findById(id); if (!current) throw new AnnouncementNotFoundError(); await announcementsRepository.deleteById(id); },
};
