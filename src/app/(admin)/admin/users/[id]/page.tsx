...
export default async function AdminUserDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <>
    <HeadingSection title={user.profile?.real_name || user.name} description="查看帳號資訊與管理端可維護的基本資料。" actions={<div className="flex flex-wrap gap-2"><UserProfileEditButton userId={user.id} profile={user.profile} /><ButtonLink href="/admin/users" variant="outline">返回使用者管理</ButtonLink></div>} />
