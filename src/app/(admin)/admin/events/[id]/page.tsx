...
export default async function AdminEventDetailPage({
  ...
  return (
    <>
      <HeadingSection
        title={event.name}
        description={`${new Date(event.start_time).toLocaleString("zh-TW")} 至 ${new Date(event.end_time).toLocaleString("zh-TW")}`}
      />
      <section className="px-4 pb-6">
        <div className="card mb-5 rounded-2xl p-5">
          <p className="whitespace-pre-wrap text-sm leading-7">
            {event.description || "未提供活動說明。"}
          </p>
        </div>
        <AttendanceManager
          eventId={event.id}
