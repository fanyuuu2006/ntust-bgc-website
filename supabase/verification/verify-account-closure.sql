...
select pg_temp.check_true(not has_function_privilege('anon', 'public.close_account(text,text)', 'EXECUTE'), 'anon forbidden');
select pg_temp.check_true(not has_function_privilege('authenticated', 'public.close_account(text,text)', 'EXECUTE'), 'authenticated forbidden');
select pg_temp.check_true(has_function_privilege('service_role', 'public.close_account(text,text)', 'EXECUTE'), 'server allowed');
...
begin
  ...
  foreach s in array array['none','pending','approved','borrowed','returned','rejected','cancelled'] loop
    ...
    insert into public.sessions(user_id,token,expires_at) values (uid,uid||'-one',now()+interval '1 day'), (uid,uid||'-two',now()+interval '1 day');
    ...
    begin
      perform public.close_account(uid||'-one','changed-hash');
      raise exception 'FAIL stale credential accepted';
    exception when sqlstate 'PCL03' then null; end;
    blocked := false;
    begin perform public.close_account(uid||'-one','qa-hash');
    ...
    if blocked then
      ...
      perform public.close_account(uid||'-one','qa-hash');
    end if;
    ...
    begin perform public.close_account(uid||'-one','qa-hash'); raise exception 'FAIL repeated closure accepted';
    exception when sqlstate 'PCL01' then null; end;
    begin insert into public.sessions(user_id,token,expires_at) values (uid,uid||'-late',now()+interval '1 day'); raise exception 'FAIL late session accepted';
    exception when sqlstate 'PCL01' then null; end;
...
begin
  ...
  insert into public.auth_credentials(user_id,password_hash) values (uid,'qa-hash');
  insert into public.sessions(user_id,token,expires_at) values (uid,'rollback-token',now()+interval '1 day');
  ...
  begin perform public.close_account('rollback-token','qa-hash'); raise exception 'FAIL missing injected error';
